  ...
  var SupportTimelineHook = {
    ...
    renderChart() {
      ...
      if (isUpdate && this.svg) {
        ...
        g = this.g;
        if (this.xAxisG) {
          const xAxis2 = d3.axisBottom(newXScale).ticks(Math.min(data.length, 10)).tickFormat(d3.format("d"));
          this.xAxisG.transition().duration(TRANSITION_DURATION).call(xAxis2);
        }
        if (this.yAxisG) {
          const yAxis2 = d3.axisLeft(newYScale).ticks(5).tickFormat(d3.format(".0%"));
          this.yAxisG.transition().duration(TRANSITION_DURATION).call(yAxis2);
        }
        ...
        if (data.length === 0) {
          svg.append("text").attr("x", width / 2).attr("y", height / 2).attr("text-anchor", "middle").attr("fill", "#6b7280").attr("font-family", "monospace").text("No data yet");
          return;
        }
        g = svg.append("g").attr("transform", `translate(${margin.left},${margin.top})`);
        xScale = newXScale;
        yScale = newYScale;
        const gridColor2 = "#333333";
        g.append("g").attr("class", "grid").selectAll("line").data([0, 0.2, 0.4, 0.6, 0.8, 1]).enter().append("line").attr("x1", 0).attr("x2", innerWidth).attr("y1", (d) => yScale(d)).attr("y2", (d) => yScale(d)).attr("stroke", gridColor2).attr("stroke-width", 1);
        this.deathThreshold = g.append("line").attr("x1", 0).attr("x2", innerWidth).attr("y1", yScale(DEATH_THRESHOLD)).attr("y2", yScale(DEATH_THRESHOLD)).attr("stroke", "#ef4444").attr("stroke-width", 2).attr("stroke-dasharray", "5,5");
        this.deathLabel = g.append("text").attr("x", innerWidth + 5).attr("y", yScale(DEATH_THRESHOLD)).attr("dy", "0.35em").attr("fill", "#ef4444").attr("font-size", "10px").attr("font-family", "monospace").text("DEATH");
        this.gradThreshold = g.append("line").attr("x1", 0).attr("x2", innerWidth).attr("y1", yScale(GRADUATION_THRESHOLD)).attr("y2", yScale(GRADUATION_THRESHOLD)).attr("stroke", "#3b82f6").attr("stroke-width", 2).attr("stroke-dasharray", "5,5");
        this.gradLabel = g.append("text").attr("x", innerWidth + 5).attr("y", yScale(GRADUATION_THRESHOLD)).attr("dy", "0.35em").attr("fill", "#3b82f6").attr("font-size", "10px").attr("font-family", "monospace").text("GRAD");
        const xAxis2 = d3.axisBottom(xScale).ticks(Math.min(data.length, 10)).tickFormat(d3.format("d"));
        this.xAxisG = g.append("g").attr("transform", `translate(0,${innerHeight})`).call(xAxis2).attr("color", "#9ca3af").selectAll("text").attr("fill", "#9ca3af").attr("font-family", "monospace");
        g.append("text").attr("x", innerWidth / 2).attr("y", innerHeight + 35).attr("text-anchor", "middle").attr("fill", "#6b7280").attr("font-size", "12px").attr("font-family", "monospace").text("CYCLE");
        const yAxis2 = d3.axisLeft(yScale).ticks(5).tickFormat(d3.format(".0%"));
        this.yAxisG = g.append("g").call(yAxis2).attr("color", "#9ca3af").selectAll("text").attr("fill", "#9ca3af").attr("font-family", "monospace");
        g.append("text").attr("transform", "rotate(-90)").attr("x", -innerHeight / 2).attr("y", -40).attr("text-anchor", "middle").attr("fill", "#6b7280").attr("font-size", "12px").attr("font-family", "monospace").text("SUPPORT");
      }
      const gridColor = "#333333";
      g.append("g").attr("class", "grid").selectAll("line").data([0, 0.2, 0.4, 0.6, 0.8, 1]).enter().append("line").attr("x1", 0).attr("x2", innerWidth).attr("y1", (d) => yScale(d)).attr("y2", (d) => yScale(d)).attr("stroke", gridColor).attr("stroke-width", 1);
      g.append("line").attr("x1", 0).attr("x2", innerWidth).attr("y1", yScale(DEATH_THRESHOLD)).attr("y2", yScale(DEATH_THRESHOLD)).attr("stroke", "#ef4444").attr("stroke-width", 2).attr("stroke-dasharray", "5,5");
      g.append("text").attr("x", innerWidth + 5).attr("y", yScale(DEATH_THRESHOLD)).attr("dy", "0.35em").attr("fill", "#ef4444").attr("font-size", "10px").attr("font-family", "monospace").text("DEATH");
      g.append("line").attr("x1", 0).attr("x2", innerWidth).attr("y1", yScale(GRADUATION_THRESHOLD)).attr("y2", yScale(GRADUATION_THRESHOLD)).attr("stroke", "#3b82f6").attr("stroke-width", 2).attr("stroke-dasharray", "5,5");
      g.append("text").attr("x", innerWidth + 5).attr("y", yScale(GRADUATION_THRESHOLD)).attr("dy", "0.35em").attr("fill", "#3b82f6").attr("font-size", "10px").attr("font-family", "monospace").text("GRAD");
      const xAxis = d3.axisBottom(xScale).ticks(Math.min(data.length, 10)).tickFormat(d3.format("d"));
      g.append("g").attr("transform", `translate(0,${innerHeight})`).call(xAxis).attr("color", "#9ca3af").selectAll("text").attr("fill", "#9ca3af").attr("font-family", "monospace");
      g.append("text").attr("x", innerWidth / 2).attr("y", innerHeight + 35).attr("text-anchor", "middle").attr("fill", "#6b7280").attr("font-size", "12px").attr("font-family", "monospace").text("CYCLE");
      const yAxis = d3.axisLeft(yScale).ticks(5).tickFormat(d3.format(".0%"));
      g.append("g").call(yAxis).attr("color", "#9ca3af").selectAll("text").attr("fill", "#9ca3af").attr("font-family", "monospace");
      g.append("text").attr("transform", "rotate(-90)").attr("x", -innerHeight / 2).attr("y", -40).attr("text-anchor", "middle").attr("fill", "#6b7280").attr("font-size", "12px").attr("font-family", "monospace").text("SUPPORT");
    ...
    renderChart() {
      ...
      if (filteredData.length === 0) {
        if (!isUpdate) {
          ...
          svg2.append("text").attr("x", width / 2).attr("y", height / 2).attr("text-anchor", "middle").attr("fill", "#6b7280").attr("font-family", "monospace").text("No contributions yet");
        }
        return;
      }
      let svg;
      ...
      if (!isUpdate) {
        pieGroup.append("text").attr("class", "center-label").attr("text-anchor", "middle").attr("dy", "-0.2em").attr("fill", "#9ca3af").attr("font-family", "monospace").attr("font-size", "12px").text("TOTAL");
        pieGroup.append("text").attr("class", "center-count").attr("text-anchor", "middle").attr("dy", "1em").attr("fill", "#ffffff").attr("font-family", "monospace").attr("font-size", "20px").attr("font-weight", "bold").text(totalCount);
      } else {
      ...
      if (!isUpdate) {
        ...
        const itemWidth = 100;
        const itemHeight = 20;
        const legendItems = legendGroup.selectAll(".legend-item").data(filteredData).enter().append("g").attr("class", "legend-item").attr("transform", (d, i) => {
          const row = Math.floor(i / itemsPerRow);
          const col = i % itemsPerRow;
          return `translate(${col * itemWidth},${row * itemHeight})`;
        });
        legendItems.append("rect").attr("width", 12).attr("height", 12).attr("fill", (d) => d.color || getRoleColor(d.role)).attr("stroke", "#ffffff").attr("stroke-width", 1);
        legendItems.append("text").attr("x", 18).attr("y", 10).attr("fill", "#9ca3af").attr("font-family", "monospace").attr("font-size", "10px").text((d) => `${formatRoleShort(d.role)} (${d.count})`);
      }
      this.svg = svg;
      this.isInitialRender = false;
    }
    ...
    renderChart() {
      ...
      if (sortedData.length === 0) {
        if (!isUpdate) {
          ...
          svg2.append("text").attr("x", width / 2).attr("y", height / 2).attr("text-anchor", "middle").attr("fill", "#6b7280").attr("font-family", "monospace").text("No trajectory data yet");
        }
        return;
      }
      ...
      if (isUpdate && this.svg) {
        ...
        this.gridX = g.append("g").attr("class", "grid-x").selectAll("line").data(xTicks).enter().append("line").attr("x1", (d) => xScale(d)).attr("x2", (d) => xScale(d)).attr("y1", 0).attr("y2", innerHeight).attr("stroke", gridColor).attr("stroke-width", 1);
        const yTicks = yScale.ticks(5);
        this.gridY = g.append("g").attr("class", "grid-y").selectAll("line").data(yTicks).enter().append("line").attr("x1", 0).attr("x2", innerWidth).attr("y1", (d) => yScale(d)).attr("y2", (d) => yScale(d)).attr("stroke", gridColor).attr("stroke-width", 1);
        ...
        if (!isUpdate) {
          ...
          this.xAxisG = g.append("g").attr("transform", `translate(0,${innerHeight})`).call(xAxis).attr("color", "#9ca3af").selectAll("text").attr("fill", "#9ca3af").attr("font-family", "monospace");
          g.append("text").attr("x", innerWidth / 2).attr("y", innerHeight + 40).attr("text-anchor", "middle").attr("fill", "#6b7280").attr("font-size", "12px").attr("font-family", "monospace").text("PC1");
          const yAxis = d3.axisLeft(yScale).ticks(5);
          this.yAxisG = g.append("g").call(yAxis).attr("color", "#9ca3af").selectAll("text").attr("fill", "#9ca3af").attr("font-family", "monospace");
          g.append("text").attr("transform", "rotate(-90)").attr("x", -innerHeight / 2).attr("y", -45).attr("text-anchor", "middle").attr("fill", "#6b7280").attr("font-size", "12px").attr("font-family", "monospace").text("PC2");
          const legendWidth = 100;
          const legendHeight = 10;
          const legendY = innerHeight + 10;
          const legendX = (innerWidth - legendWidth) / 2;
          const defs = svg.append("defs");
          const gradient = defs.append("linearGradient").attr("id", "trajectory-legend-gradient").attr("x1", "0%").attr("x2", "100%");
          gradient.append("stop").attr("offset", "0%").attr("stop-color", "#06b6d4");
          gradient.append("stop").attr("offset", "100%").attr("stop-color", "#ffffff");
          g.append("rect").attr("x", legendX).attr("y", legendY).attr("width", legendWidth).attr("height", legendHeight).attr("fill", "url(#trajectory-legend-gradient)").attr("stroke", "#ffffff").attr("stroke-width", 1);
          g.append("text").attr("x", legendX).attr("y", legendY + legendHeight + 12).attr("fill", "#6b7280").attr("font-size", "10px").attr("font-family", "monospace").text(`C${cycleExtent[0]}`);
          g.append("text").attr("x", legendX + legendWidth).attr("y", legendY + legendHeight + 12).attr("text-anchor", "end").attr("fill", "#6b7280").attr("font-size", "10px").attr("font-family", "monospace").text(`C${cycleExtent[1]}`);
          g.append("text").attr("x", legendX + legendWidth / 2).attr("y", legendY - 3).attr("text-anchor", "middle").attr("fill", "#9ca3af").attr("font-size", "10px").attr("font-family", "monospace").text("CYCLE");
        }
      }
      this.svg = svg;
      this.g = g;
